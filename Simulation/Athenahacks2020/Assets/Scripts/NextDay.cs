    ...
    public void calculatingNextDay(){
        ...
        for (int i = 0; i < Statistics.numPeople; i++){
            ...
            while (curChanceToInteract < chanceToInteract){
                ...
                if (CrowdController.arr_people[indexOfInteract].hasVirus){
                    ...
                    if (Random.Range(0.0f,1.0f) < chanceToCatch){
            ...
            if (CrowdController.arr_people[i].hasVirus && CrowdController.arr_people[i].healthState == 1){
                if (Random.Range(0.0f,1.0f) < Statistics.globalChanceOfGoingToTest){
                    CrowdController.arr_people[i].healthState = 2;
                }
            }

            if (CrowdController.arr_people[i].healthState == 2){
                if (Random.Range(0.00f,1.00f) <= 1){
